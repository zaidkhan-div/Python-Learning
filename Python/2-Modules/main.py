...
with open("note.txt","r") as file1: # r => read
    print(file1.read())
...
with open("note.txt", "w+") as file3: # in this case we can read & write with "w+" & "r+"
    file3.write("We can write here")
    print(file3.read()) # also we can read this
    
with open("note.txt", "w+") as file3: # if the file does'nt exist this will creat and we can read and write both but with "r+" we can create file but can't write 
    file3.write("Hello World")
    file3.seek(0) # seek method use to set the cursor 
    print(file3.read())

with open("note.txt", "a+") as file4: # "a" mode represent append
